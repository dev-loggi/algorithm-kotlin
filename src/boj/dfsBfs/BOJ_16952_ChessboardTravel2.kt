 ...
 * 16952 체스판 여행 2
 * */
class BOJ_16952_ChessboardTravel2 : BOJSolution(info(), testCases()) {
    ...
    data class Node(
        ...
        fun getNext(n: Int = this.n, piece: Int = this.piece, row: Int = this.row, col: Int = this.col, convert: Int = this. convert): Node {
            return Node(n, piece, row, col, time + 1, convert)
        }
    }
    ...
    override fun runEachSolution() {
        ...
        solution(r0, c0).let { println(it.joinToString(" ")) }
        queue.clear()
    }

    fun solution(r0: Int, c0: Int): IntArray {
        // visited[n][p][r][c]
        ...
        for (p in PIECES) {
            ...
            queue.offer(Node(1, p, r0, c0, 0, 0))
        }
        ...
        while (queue.isNotEmpty()) {
            ...
            for (p in PIECES) {
                ...
                queue.offer(cur.getNext(piece = p, convert = cur.convert + 1))
            }
        ...
        return temp.minByOrNull { it.convert }!!.let { intArrayOf(it.time, it.convert) }
    }
...
private fun info() = BOJSolution.Info(
    ...
    problemUrl = "https://www.acmicpc.net/problem/16952",
    solutionUrl = ""
)
